...
**Answer**: The distribution shows that the majority of people who took the test had warmth scores between around 13 to around 17. The bar plot works because it shows how the majority of people scored. 
...
**Answer**: The range of the confidence interval is bigger in the Hungary dataset than in the entire dataset. The smaller data sample contains less responses than the entire dataset, and therefore the confidence interval will be less reliable, leading to the range being larger.
...
**Answer**: The difference between men and women's average score is not statistically significant because one end of the confidence is negative and one is positive. The difference, in the small chance that there is a difference, shows that Hungarian men might be slightly less warm than Hungarian women. This would confirm traditional gender stereotypes. 
...
```{r}
thirtytofourtynine <- filter (pf, age >=30, age <=49)
```
...
```{r}
thirtytofourtynine$fourties <- thirtytofourtynine$age >= 40
```
...
```{r}
model <- lm_basic(warmth ~ 1 + fourties, data = thirtytofourtynine)
reg_table (model, level = 0.95)
```
...
**Answer**: Yes, there is a statistically significant difference between the two decades because both values of the confidence interval are positive. I am not aware of any age stereotypes regarding warmth. 
...
**Answer**: AU (Intercept) has the highest score of warmth. IN has the lowest score of warmth.
...
**Answer**: No, the slope is not statistically significantly different than 1. This is saying that sensitivity is less sensitive to the baseline than you would expect. 
...
**Answer**: The gender slope is fairly significantly different than the baseline and the intercept.
...
**Answer**: They have the same slope, with the male score being constantly slightly lower than the female score.
