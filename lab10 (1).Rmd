...
Above is a plot showing the frequency of the swear word "fuck" in the movies of Quentin Tarantino. We can see from the graph that the use of this particular swear word is most frequent in the movies Pulp Fiction and Reservoir Dogs.

```{r}
tarantino_django <- filter(tarantino, movie == "Django Unchained")
```

```{r}
ggplot(tarantino_django, aes (word)) +
  geom_bar(aes(fill = word), position = "dodge")
```

Above is a plot showing the frequency of each swear word in the movie Django Unchained. As we can see from the graph, the n-word is by far the most frequently used swear word in this movie.

```{r}
tarantino_pulp_fiction <- filter(tarantino, movie == "Pulp Fiction", word %in% c("fuck", "shit", "asshole"))
```

```{r}
ggplot(tarantino_pulp_fiction, aes(minutes_in)) +
  geom_histogram (aes(fill = word), position = "dodge")
```

Shown above is a graph of the frequency of the three swear words "asshole," "fuck," and "shit" in the movie Pulp Fiction. From the data, it seems as though the swear words being to be used more frequently between 100 and 150 minutes.

```{r}
tarantino_n_word <- filter(tarantino, word == "n-word")
```

```{r}
ggplot(tarantino_n_word, aes(movie)) +
  geom_bar (aes(fill = movie))
```

The graph above shows the frequency of the n-word in Tarantino's movies. As we can see on the graph, the n-word is only used in four of Tarantino's many movies. 