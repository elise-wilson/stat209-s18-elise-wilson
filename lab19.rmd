...
**Answer**: No, it is not noise in the data, because both of the confidence intervals are above 3.7, and 3.7 is not in between them. 
...
**Answer**: This assumption may not be true in this case because likely the people sampling the burrito places picked the best burrito places or picked which ones they wanted to go to beforehand, and therefore the ones they went to were not randomly sampled or unbiased.
...
**Answer**: The average yelp score was higher by 0.05699 points if chips were included.
...
**Answer**: Because the range goes from a negative to a positive number, and zero is included in this range, there is no way to tell whether or not having chips included is helping or hurting the review score. 
...
**Answer**: There is not enough evidence because the range of confidence goes from a negative reaction to a positive reaction to no reaction at all depending on whether or not the burrito restaurant is high synergy or not.
...
**Answer**: Yes, there is evidence because the range stays within positive values, meaning that there will always be a differences in scores if the burrito place has high synergy.
...
**Answer**: It predicts that the overall score will be lower by at least 1.1678 of a score than the scores of those burrito places with high synergy.

What does the model predict will be the overall score of a location with
high synergy?

**Answer**: The overall score will be 1.1678 scores higher than the overall score.

Finally, fit a regression model that predicts the overall score as a function
of the variable `synergy` coded as a factor.

```{r}
model <- lm_basic (yelp ~ 1 + synergy, data = burrito) 
reg_table (model, level = 0.95)
```
...
**Answer**: It predicts that the overall score will be higher than those who have a lower synergy score.
