...
```{r}
ggplot (bird_type, aes(egg_mass_mean, male_mass_mean)) +
  geom_point () +
  scale_x_log10 () +
  scale_y_log10 ()
```
...
```{r}
bird_type_five <- filter (bird_type, n >= 5)
```
...
ggplot (bird_type, aes(egg_mass_mean, male_mass_mean)) +
  ...
  scale_x_log10 () +
  scale_y_log10 () +
  geom_point (aes(color = "red"), data = bird_type_five) +
  geom_text (data = bird_type_five, aes (label = type)) +
  xlab ("Egg Mass Mean") +
  ylab ("Male Mass Mean") +
  ggtitle ("Comparison of Male Bird Masses to the Egg Masses") +
  labs (color = "Bird Types with Five or More Birds")
```
