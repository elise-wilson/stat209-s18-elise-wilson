...
As we can see from the graph below, certain states did not have a drastically different number of murders recorded between the two years in questions, 2014 and 2015. But there are two outliers: Chicago and Baltimore. This is not surprising, as both cities have been notorious for their murder rates throughout history, not simply in these two years. Additionally, it is interesting to note that Baltimore has the biggest change between the years, while Chicago has the highest number of murders between the two years while still having quite a big change as well.

```{r}
ggplot (murder, aes(murders_2014, murders_2015)) +
  geom_point (aes(size = change, color = state)) +
  annotate ("text", x = 411, y = 478, label = "Chicago") + 
  annotate ("text", x = 211, y = 344, label = "Baltimore") +
  xlab ("Murders in 2014") +
  ylab ("Murders in 2015") +
  labs (size = "Change in Number of Murders") +
  labs (color = "State Name") +
  ggtitle ("Changes in Murder Rates by State, 2014-2015")
