...
**Answer**: This means that about 17.25% of the crimes reported actually resulted in an arrest.
...
**Answer**: This means that 27.51% of the reported crimes were reported as thefts.
...
**Answer**: This means that out of all the crimes reported in the dataset, there were 38816 reported thefts.
...
**Answer**: This means that there are 36.55 thefts per 1000 households.
...
**Answer**: It is bigger. This means that 25.7% of the crimes reported in Humboldt Park ended in arrests, as opposed to 17.5% of the crimes ending in arrests from the entire dataset. Yes, we can safely compare these measurements.
...
**Answer**: This number is smaller than the theft reports from the entire dataset. In this dataset, only 18.77% of the reported crimes were reported as thefts, as opposed to 27.5% reported as thefts in the entire dataset. The two measurements are measuring the same thing, so therefore we can compare the measurements.
...
**Answer**: This is larger than the measurement over the entire dataset. We can compare these two measurements because the size of the neighborhood is standardized by the number of households.
...
**Answer**: The area_number variable. 
...
**Answer**: There are generally the same number of thefts no matter what you income is, with the exception of one outlier where the median income is about $65000, and the theft rate is 350.
...
**Answer**: The first reason that the Loop is an outlier is that it is there are a lot of people in the area, including tourists and businessmen. But, not a lot of people live there, so the crime rate is high for the number of households that are there. We are using the number of households as a determining factor for how many people are there, and it is not a good determining factor. But also, the theft rate is high because the Loop is a big commercial district, so there are a lot of tourists and businessmen to steal from by pickpocketing, etc. 
