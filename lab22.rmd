...
**Answer**: It differs by about 3.7 score units each time, meaning they are very similar. Both slopes are statistically significant. 
...
**Answer**: One reason some may finish quickly is because they are not really reading the questions and are not being truly honest in their answers, and some may take longer because they are paying attention and being very precise.
...
```{r}
model <- lm_basic(warmth ~ 1 + liveliness + tension + baseline, data = pf)
reg_table(model, level = 0.95)
```
...
**Answer**: Yes, they do match what I would have expected, because I would expect that the liveliness score would be higher or closer to the score of the warmth trait and that the tension trait would be lower and farther from the score of the warmth trait.

Take the previous model and add the variables `speed`, `gender`, and
`country`, the latter lumped into 5 categories.

```{r}
model <- lm_basic(warmth ~ 1 + liveliness + tension + baseline + speed + gender + fct_lump(country, n =5), data = pf)
reg_table(model, level = 0.95)
```

Does this change the slopes for your two traits much?

**Answer**: No, it does not change the slopes for my two traits very much. The changed only slightly.

Summarize the previous model (specifically the relationship between the
three traits) in words.

**Answer**: The relationship between the three traits shows that the average scores of these three traits do not change very much based on speed, gender, and country, which could show a common understanding of how people as a whole view their own traits. 
